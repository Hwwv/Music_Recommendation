...
def recall_at_k(recommendations: Mapping[int, list[str]], truth: Mapping[int, str], k: int) -> float:
...
def ndcg_at_k(recommendations: Mapping[int, list[str]], truth: Mapping[int, str], k: int) -> float:
    users = [u for u in truth if u in recommendations]
    if not users:
        return 0.0
    total = 0.0
...
def ndcg_at_k_multiple(recommendations: Mapping[int, list[str]], truth: Mapping[int, list[str]], k: int) -> float:
    users = [u for u in truth if u in recommendations]
    if not users:
        return 0.0
    total = 0.0
...
def catalog_coverage(recommendations: Mapping[int, list[str]], catalog: set[str], k: int) -> float:
    exposed = {item for values in recommendations.values() for item in values[:k]}
    return len(exposed) / len(catalog) if catalog else 0.0


def intra_list_diversity(recommendations: Mapping[int, list[str]], features: Mapping[str, list[float]], k: int) -> float:

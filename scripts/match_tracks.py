...
def main() -> None:
    ...
    con.execute(
        ...
        SELECT 'exact_v1', 'listening_track_keys', count(*) FROM listening.listening_track_keys
        UNION ALL SELECT 'exact_v1', 'candidate_rows', count(*) FROM track_match_candidates
        UNION ALL SELECT 'exact_v1', 'accepted_track_keys', count(*) FROM track_match_decisions WHERE decision = 'accepted'
        UNION ALL SELECT 'exact_v1', 'ambiguous_track_keys', count(*) FROM track_match_decisions WHERE decision = 'ambiguous'
        UNION ALL SELECT 'exact_v1', 'unmatched_track_keys', count(*) FROM track_match_decisions WHERE decision = 'rejected'
        UNION ALL SELECT 'exact_v1', 'integrated_interactions', count(*) FROM interactions_integrated
        UNION ALL SELECT 'exact_v1', 'integrated_users', count(DISTINCT user_id) FROM interactions_integrated
        UNION ALL SELECT 'exact_v1', 'integrated_spotify_tracks', count(DISTINCT spotify_track_id) FROM interactions_integrated
        UNION ALL SELECT 'exact_v1', 'users_with_5_plus_matches', count(*) FROM (SELECT user_id FROM interactions_integrated GROUP BY user_id HAVING count(*) >= 5)
        UNION ALL SELECT 'exact_v1', 'users_with_10_plus_matches', count(*) FROM (SELECT user_id FROM interactions_integrated GROUP BY user_id HAVING count(*) >= 10)
        UNION ALL SELECT 'exact_v1', 'users_with_20_plus_matches', count(*) FROM (SELECT user_id FROM interactions_integrated GROUP BY user_id HAVING count(*) >= 20)
        UNION ALL SELECT 'exact_v1', 'interactions_merging_multiple_listening_keys', count(*) FROM interactions_integrated WHERE merged_listening_key_count > 1
        """
    )
    ...
    for metric, value in con.sql(
        "SELECT metric, value FROM integration_audit WHERE run_id='exact_v1' ORDER BY metric"
    ).fetchall():

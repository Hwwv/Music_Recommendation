...
Run from the project root:
    python scripts/run_cbm_country.py --country US --country_sample_fraction 0.15
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--global_weights", type=float, nargs="+", default=GLOBAL_WEIGHTS)
    parser.add_argument("--k_for_kmeans", type=int, nargs="+", default=KMEANS)
...
def main() -> None:
    ...
    validation_truths = {u: v for u, v in validation_truths.items() if u in sampled_users}
    validation_users = sorted(validation_truths.keys())
    print(f"Validation users after country filter: {len(validation_users):,}")
    if not validation_users:
        raise SystemExit("no validation users left after filtering; try a larger --country_sample_fraction")

    results_cbm: dict[float, dict] = defaultdict(dict)
    for alpha in args.alphas:
        for gw in args.global_weights:
            for km in args.k_for_kmeans:
                ...
                for user in validation_users:
                    recommendations[user] = multi_cbm_recommender.recommend(user, k=max(args.ks))
    ...
    output_path.write_text(json.dumps(results_cbm, indent=2, sort_keys=True) + "\n", encoding="utf-8")

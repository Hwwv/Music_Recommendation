...
def main() -> None:
    ...
    con.execute("DELETE FROM integration_audit WHERE run_id = 'feature_cluster_v2'")
        ...
        WITH primary_artists AS (
            ...
            SELECT
                ...
                sha256(to_json(struct_pack(
                    ...
                    track_genre := coalesce(t.track_genre, '')
    ...
    con.execute(
        ...
        SELECT 'feature_cluster_v2', 'spotify_tracks', count(*) FROM project_tracks
        UNION ALL SELECT 'feature_cluster_v2', 'spotify_feature_clusters', count(*) FROM spotify_feature_clusters
        UNION ALL SELECT 'feature_cluster_v2', 'multi_id_feature_clusters', count(*) FROM spotify_feature_clusters WHERE member_count > 1
        UNION ALL SELECT 'feature_cluster_v2', 'accepted_listening_keys', count(*) FROM listening_feature_decisions WHERE decision = 'accepted'
        UNION ALL SELECT 'feature_cluster_v2', 'accepted_multi_id_keys', count(*) FROM listening_feature_crosswalk WHERE spotify_id_count > 1
        UNION ALL SELECT 'feature_cluster_v2', 'ambiguous_listening_keys', count(*) FROM listening_feature_decisions WHERE decision = 'ambiguous'
        UNION ALL SELECT 'feature_cluster_v2', 'unmatched_listening_keys', count(*) FROM listening_feature_decisions WHERE decision = 'rejected'
        UNION ALL SELECT 'feature_cluster_v2', 'feature_interactions', count(*) FROM feature_interactions_integrated
        UNION ALL SELECT 'feature_cluster_v2', 'feature_users', count(DISTINCT user_id) FROM feature_interactions_integrated
        UNION ALL SELECT 'feature_cluster_v2', 'represented_feature_clusters', count(DISTINCT feature_cluster_id) FROM feature_interactions_integrated
        UNION ALL SELECT 'feature_cluster_v2', 'users_with_5_plus_features', count(*) FROM (SELECT user_id FROM feature_interactions_integrated GROUP BY user_id HAVING count(*) >= 5)
        UNION ALL SELECT 'feature_cluster_v2', 'users_with_10_plus_features', count(*) FROM (SELECT user_id FROM feature_interactions_integrated GROUP BY user_id HAVING count(*) >= 10)
        UNION ALL SELECT 'feature_cluster_v2', 'users_with_20_plus_features', count(*) FROM (SELECT user_id FROM feature_interactions_integrated GROUP BY user_id HAVING count(*) >= 20)
        """
    )
    ...
    for metric, value in con.sql(
        "SELECT metric, value FROM integration_audit WHERE run_id='feature_cluster_v2' ORDER BY metric"
    ).fetchall():
